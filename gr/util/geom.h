...
struct RigidTransform {
	...
	using Box = Box<Vec>;
	using OrientedBox = OrientedBox<Vec>;
	...
	constexpr RigidTransform(Vec const &pos = Vec(0), Rot const &rot = Rotation::Identity(), Num scale = Num(1)) : _position(pos), _orientation(Rotation::Normalize(rot)), _scale(scale) {}

	constexpr bool IsIdentity(Num eps = NumericTraits<Num>::Eps) const 
	{
		return IsZero(_position, eps) && IsEqual(rot, Rot::Identity(), eps) && IsEqual(_scale, Num(1), eps);
	}

	constexpr RigidTransform Inverse() const 
	{
		Rot invRot = Rotation::Inverse(_orientation);
		Num invScale = 1 / _scale;
		return RigidTransform(-Rotation::Rotate(invRot, _position * invScale), invRot, invScale);
	}

	constexpr Vec TransformVector(Vec const &v) const { return Rotation::Rotate(_orientation, v * _scale); }
	constexpr Vec TransformPoint(Vec const &v) const { return TransformVector(v) + _position; }

	constexpr Vec InverseTransformVector(Vec const &v) const { return Rotation::Rotate(Rot::Inverse(_orientation), v / _scale); }
	constexpr Vec InverseTransformPoint(Vec const &v) const
	{
		Rot invRot = Rotation::Inverse(_orientation);
		Num invScale = 1 / _scale;
		return Rotation::Rotate(invRot, v * invScale) - Rot::Rotate(invRot, _position * invScale);
	}

	constexpr OrientedBox TransformToOrientedBox(Box const &box) const { return OrientedBox(TransformPoint(box.GetCenter()), box.GetSize() * _scale / 2, _orientation); }
	constexpr Box Transform(Box const &box) const { return TransformToOrientedBox(box).GetBox(); }
	constexpr OrientedBox Transform(OrientedBox const &ob) const { return OrientedBox(TransformPoint(ob._center), ob._halfSize * _scale, Rotation::Compose(_orientation, ob._orientation)); }
	constexpr Sphere Transform(Sphere const &sphere) const { return Sphere(TransformPoint(sphere._center), sphere._radius * _scale); }
	constexpr Plane Transform(Plane const &plane) const { return Plane(TransformVector(plane._normal), TransformPoint(plane.GetAnyPoint())); }
	constexpr Line Transform(Line const &line) const { return Line(TransformPoint(line._origin), TransformVector(line._direction)); }
	...
	constexpr Polygon<PointArray> Transform(Polygon<PointArray> const &poly) const
	{
		...
		TransformPoints(poly._points, tranformed._points);
		return transformed;
	}
};
...
struct Box {
	...
	using Num = typename VecTraits<Vec>::ElemType;
	static constexpr int Dim = VecTraits<Vec>::Length;
	using BVec = typename VecTraits<Vec>::WithElemType<bool>;
	using Line = Line<Vec>;
	...
	constexpr Vec GetCenter() const { return (m_min + m_max) / 2; }
	...
	constexpr Line GetEdge(int edgeInd) const
	{
		glm::ivec2 pointInds = BoxTraits<Dim>::GetEdgePointIndices(edgeInd);
		return Line(GetPoint(pointInds[0]), GetPoint(pointInd[1]));
	}

	constexpr int GetNumEdgeDirections() const { return Dim; }
	constexpr int GetNumEdgeDirections(int dirInd) const { return VecCardinal(dirInd); }

	constexpr int GetNumSideDirections() const { return Dim; }
	constexpr int GetSideDirection(int dirInd) const { return VecCardinal(dirInd); }
};
...
struct OrientedBox {
	...
	using Box = Box<Vec>;
	using Line = Line<Vec>;
	using RigidTransform = RigidTransform<Vec>;

	Rot _orientation;
	Vec _center, _halfSize;

	constexpr OrientedBox() = default;
	constexpr OrientedBox(Vec const &center, Vec const &halfSize, Rot const &orientation = Rotation::Identity()) : _orientation(orientation), _center(center), _halfSize(halfSize) { ASSERT(IsValid()); }
	constexpr OrientedBox(Box const &box, Rot const &orientation = Rotation::Identity()) : _orientation(orientation), _center(box.GetCenter()), _halfSize(box.GetSize() / 2) { ASSERT(IsValid()); }
	...
	constexpr Vec FromBoxPointOffset(Vec const &v) const { return Rotation::Rotate(_orientation, v) + m_center; }
	...
	constexpr int GetNumEdgeDirections() const { return Dim; }
	constexpr int GetNumEdgeDirections(int dirInd) const { return Rotation::Rotate(_orientation, VecCardinal(dirInd)); }

	constexpr int GetNumSideDirections() const { return Dim; }
	constexpr int GetSideDirection(int dirInd) const { return Rotation::Rotate(_orientation, VecCardinal(dirInd)); }

	constexpr Box GetUntransformedBox() const
	{
		return Box::FromCenterHalfSize(_center, _halfSize);
	}

	constexpr RigidTransform GetTransformToBox() const 
	{ 
		ASSERT(IsValid());
		return RigidTransform(_center, _orientation).Inverse();
	}
	constexpr Vec ToBoxPointOffset(Vec const &v) const { GetTransformToBox().TransformPoint(v); }

	constexpr Box GetBox() const
	{
		Vec size(0);
		...
		for (int i = 0; i < (1 << (Dim - 1)); ++i) {
			...
			Vec rotPointOffs = Rotation::Rotate(_orientation, boxPointOffs);
			size = glm::max(size, glm::abs(rotPointOffs));
		}

		return Box::FromCenterHalfSize(_center, size);
	}
	...
	constexpr bool Intersects(Box const &box) const
	{
		return SatTest<Box, OrientedBox>::Intersect(box, *this);
	}
...
struct Sphere {
	...
	constexpr bool Intersects(Box const &box) const { return Contains(box.GetClosestPoint(_center)); }
	constexpr bool Intersects(OBox const &ob) const 
	{
		RigidTransform toBox = ob.GetTransformToBox();
	...
	constexpr Sphere GetUnion(Sphere const &other) const
	{
		...
		if (dist + other._radius <= _radius)
			return *this;
		centerOther /= dist;
		Vec opposite = _center - centerOther * _radus;
		Vec otherOpposite = other._center + centerOther * _other._radius;
...
struct Plane {
	...
	template <typename PointsArray>
	static constexpr Plane GetPlane(PointsArray const &points) { return Plane(GetNormal(points), p[0]); }
	...
	constexpr Line GetIntersectionLine(Plane const &other) const
	{
	...
	constexpr Plane GetPerpendicularPlaneThroughLine(Line const &line) const
	{
		Vec norm = glm::cross(line._direction, _normal);
		return Plane(norm, line._origin);
	}

	constexpr Interval Eval(Box const &box) const
	{
	...
	constexpr Interval Eval(OBox const &ob) const
	{
		ASSERT(ob.IsValid());
		...
		for (int i = 0; i < (1 << (Dim - 1)); ++i) {
			...
			extentDir = Rotation::Rotate(ob._orientation, extentDir);
			extentEval = glm::max(extentEval, glm::abs(glm::dot(extentDir, _normal)));
		}
		Interval result(centerEval - extentEval, centerEval + extentEval);
		return result;
	}

	constexpr Interval Eval(Sphere const &sphere) const
	{
...
struct Line {
	...
	constexpr Num GetIntersectionValue(Plane const &plane) const
	{
		...
		Num t = (-_d - glm::dot(_origin, plane._normal)) / dotDirNorm;
		ASSERT(IsZero(plane.Eval(GetPoint(t))));
		return t;
	}

	// the negative side of the plane is considered "inside"
	constexpr Interval GetIntersection(Plane const &plane) const
	{
	...
	constexpr Interval GetIntersection(Box const &box) const
	{
	...
	constexpr Interval GetIntersection(OBox const &ob) const 
	{
		Line boxLine = ob.GetTransformToBox().Transform(*this);
		return boxLine.GetIntersection(ob.GetUntransformedBox());
	}

	constexpr Interval GetIntersection(Sphere const &sphere) const
	{
...
struct Polygon {
	...
	constexpr Plane GetPlane() const 
	{
		if (_points.size() < 3)
			return Plane();
		return Plane::GetPlane(_points);
	}
	...
	constexpr bool IsConvex() const
	{
		...
		for (int i = 0; i < _points.size(); prev = i++) {
			Line edge = Line::FromPoints(_points[prev], _points[i]);
			Plane edgePlane = plane.GetPerpendicularPlaneThroughLine(edge);
	...
	constexpr Vec GetClosestPoint(Vec v) const
	{
		...
		auto projValueIfOutside = [&](int edgeFirstInd) {
			edge = Line::FromPoints(_points[edgeFirstInd % _points.size()], _points[(edgeFirstInd + 1) % _points.size()]);
			Plane edgePlane = plane.GetPerpendicularPlaneThroughLine(edge);
	...
	constexpr bool Intersects(Sphere const &sphere) const
	{
...
struct PlaneEval {
	...
	static constexpr Interval Eval(Plane const &plane, Shape const &shape)
	{
...
struct SatTest {
	...
	static constexpr bool Intersects(Shape0 const &shape0, Shape1 const &shape1)
	{
		...
		for (int i = 0; i < dirs; ++i) {
			Plane axis(shape0.GetSideDirection(i), p);
		...
		int dirs = shape1.GetNumSideDirections();
		for (int i = 0; i < dirs; ++i) {
			Plane axis(shape1.GetSideDirection(i), p);
			Interval int0 = PlaneEval<Shape0>::Eval(axis, shape0);
			Interval int1 = PlaneEval<Shape1>::Eval(axis, shape1);
			if (!int0.Intersects(int1))
				return false;
...
struct hash<util::Sphere<VecType>> {
	...
	{
		size_t hash = hash<SphereType::Vec>()(b._center);
		hash = 31 * hash + hash<SphereType::Num>()(b._radius);
		return hash;
	}
};
