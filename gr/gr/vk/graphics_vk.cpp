...
std::shared_ptr<Image> GraphicsVk::CreateImage(Image::Usage usage, ColorFormat format, glm::uvec4 size, uint32_t mipLevels)
{
  vk::Format vkFormat = ImageVk::ColorFormat2Vk(format);
  auto image = std::make_shared<ImageVk>(*_device, vkFormat, size, mipLevels, usage);
  return image;
}
