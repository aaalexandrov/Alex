...
struct ParseNode {
	...
	Token const *GetToken(int32_t i) const { return i < GetContentSize() ? _content[i].GetToken() : nullptr; }
	ParseNode const *GetSubnode(int32_t i) const { return i < GetContentSize() ? _content[i].GetNode() : nullptr; }
