...
public class GLESShader {
	...
	public boolean initUniformsAndSamplers() {
		...
		int[] typeBuf = new int[1], elemsBuf = new int[1], nameLenBuf = new int[1];
		for (i = 0; i < uniformCount; i++) {
			GLES20.glGetActiveUniform(mProgram, i, nameBufLen, nameLenBuf, 0, elemsBuf, 0, typeBuf, 0, nameBuf, 0);
			if (GLES20.glGetError() != GLES20.GL_NO_ERROR) {
	...
	public boolean initAttribs() {
		...
		for (i = 0; i < attribCount; i++) {
			GLES20.glGetActiveAttrib(mProgram, i, nameBufLen, nameLenBuf, 0, elemsBuf, 0, typeBuf, 0, nameBuf, 0);
			if (GLES20.glGetError() != GLES20.GL_NO_ERROR) {
